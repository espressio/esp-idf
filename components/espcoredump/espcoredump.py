...
class ESPCoreDumpLoader(object):
    ...
    def create_corefile(self, core_fname=None, off=0, rom_elf_path=None):
        ...
        if rom_elf_path and os.path.exists(rom_elf_path):
            rom_elf = ESPCoreDumpElfFile(rom_elf_path)
...
class ESPCoreDumpFlashLoader(ESPCoreDumpLoader):
    ...
    def create_corefile(self, core_fname=None, rom_elf_path=None):
        ...
        return super(ESPCoreDumpFlashLoader, self).create_corefile(core_fname, off=self.ESP32_COREDUMP_FLASH_MAGIC_SZ, rom_elf_path=rom_elf_path)
...
def dbg_corefile(args):
    ...
    if not args.core:
        ...
        core_fname = loader.create_corefile(args.save_core, rom_elf_path=args.rom_elf)
        if not core_fname:
...
def main():
    ...
    parser_debug_coredump.add_argument('--rom-elf', '-r', help='Path to ROM ELF file.', type=str, default='eagle.rom.out')
    ...
    parser_info_coredump.add_argument('--rom-elf', '-r', help='Path to ROM ELF file.', type=str, default='eagle.rom.out')
